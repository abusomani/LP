...
int main()
{
	...
	while(state1!=NULL){
		float t=0;
		for(int i=0;i<state1->m;i++){
			for(int j=0;j<state1->m;j++)
			{
				t +=state1->S[i][j];
//			cout << t <<"  ";
			}
		}
		F[state1->no][0]=t/(state1->m*state1->m);
		state1=state1->next;
	} 
	...
	cout<<"\n\n\nEnter element which we want to check\n";
	int A1[p];
	for(int i=0;i<p;i++){
		cin>>A1[i];
	}
