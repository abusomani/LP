...
string gram[MAX][MAX];  // grammar rakhar jonyo //to store entered grammar
string dpr[MAX];
int p,np;       //np-> number of productions
 
inline string concat( string a, string b)  //nonterminal joore debe //concatenates unique non-terminals
{
    int i;
    string r=a;
    for(i,0,b.length())
...
inline void break_gram(string a)   //daan diker grammar alada korbe //seperates right hand side of entered grammar
{
    int i;
    p=0;
    while(a.length())
